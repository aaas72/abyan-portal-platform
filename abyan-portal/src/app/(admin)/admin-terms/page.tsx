...
import { TermsSectionForm } from "@/components/admin/features/terms/TermsSectionForm";

type TabId = "intro" | "usageRules" | "intellectualProperty" | "disclaimer";
...
export default function AdminTermsPage() {
  ...
  // Form
  const sectionForm = useForm<TermsItem>({
    resolver: zodResolver(TermsItemSchema),
  });

  const loadData = async () => {
    setIsLoading(true);
    const data = await TermsService.getAdminTermsContent();
  ...
  const openEditDrawer = (item: any) => {
    setEditingItem(item);
    const idx = termsData ? (termsData[activeTab] as any[]).indexOf(item) : -1;
    setEditingIndex(idx);
    sectionForm.reset({
      title: item.title || "",
  ...
  const saveToBackend = async (newData: TermsContent) => {
    if (isSaving) return;
    setIsSaving(true);
    try {
      const response = await TermsService.updateTermsContent(newData);
      if (!response.success) throw new Error("Failed to save");
      const freshData = await TermsService.getAdminTermsContent();
      setTermsData(freshData || newData);
  ...
  const handleSave = async (data: TermsItem) => {
    if (!termsData) return;
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader title="إدارة شروط وأحكام الاستخدام" />
        ...
        tabs={[
          {
            ...
            count: termsData?.intro?.length,
          },
          {
            id: "usageRules",
            label: "ضوابط التصفح والاستخدام",
            count: termsData?.usageRules?.length,
          },
          {
            id: "intellectualProperty",
            label: "الملكية والهوية الرقمية",
            count: termsData?.intellectualProperty?.length,
          },
          {
            id: "disclaimer",
            label: "المسؤولية وإخلاء الطرف",
            count: termsData?.disclaimer?.length,
          },
        ]}
      />
      ...
      <AdminDataTable
        columns={getColumns()}
        data={termsData && Array.isArray(termsData[activeTab]) ? termsData[activeTab] : []}
        isLoading={isLoading}
      ...
      >
        <div className="p-6">
          ...
          >
            <TermsSectionForm form={sectionForm} />
          </form>
        </div>
      </AdminDrawer>
    </div>
